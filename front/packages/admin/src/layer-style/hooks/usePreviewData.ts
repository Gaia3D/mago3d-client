...
export const usePreviewData = (asset: LayerAsset | null) => {
  ...
  useEffect(() => {
    ...
    const fetch = async () => {
      try {
        const url = `${import.meta.env.VITE_GEOSERVER_WFS_SERVICE_URL}service=WFS&version=2.0.0&request=GetFeature&typeName=${asset.properties.layer.resource.name}&outputFormat=application/json`;
